...
bot.on("message:text", async ctx => {
  ...
  if (state === "ub_code") {
    ...
    const ubs  = UB_ST[uid];
    if (!ubs || !ubs.client) { clearState(uid); await ctx.reply("❌ Сессия устарела. Начните заново.", { reply_markup: kbMain(uid) }); return; }
    try {
      await ubs.client.signIn({ phoneNumber: ubs.phone, phoneCodeHash: ubs.phoneCodeHash, phoneCode: code });
  ...
  if (state === "ub_2fa") {
    const ubs = UB_ST[uid];
    if (!ubs || !ubs.client) { clearState(uid); await ctx.reply("❌ Сессия устарела.", { reply_markup: kbMain(uid) }); return; }
    try {
      await ubs.client.signInWithPassword({ password: text.trim() });
      const sessStr = ubs.client.session.save();
      const me      = await ubs.client.getMe();
      db.prepare(`
        INSERT OR REPLACE INTO userbot_sessions
