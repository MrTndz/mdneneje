...
async function callAI(messages) {
  ...
  console.log(`[AI] GROQ_KEY: ${GROQ_KEY ? "✅ Установлен" : "❌ Пусто"}`);
  console.log(`[AI] GEMINI_KEY: ${GEMINI_KEY ? "✅ Установлен" : "❌ Пусто"}`);
  
  if (GROQ_KEY) {
    try {
      console.log(`[AI] Вызов Groq API...`);
      ...
      console.log(`[AI] Groq ответ: статус ${r.status}`);
      
      if (r.ok) {
        const data = await r.json();
        console.log(`[AI] ✅ Groq успешно! Длина ответа: ${data.choices[0].message.content.length} символов`);
        return data.choices[0].message.content;
      }
      
      const err = await r.text();
      console.error("[AI Groq Error]", r.status, err.slice(0, 200));
    } catch(e) { 
  ...
  if (GEMINI_KEY) {
    try {
      console.log(`[AI] Вызов Gemini API...`);
      ...
      console.log(`[AI] Gemini ответ: статус ${r.status}`);
      
      if (r.ok) {
        const data = await r.json();
        const response = data.candidates[0].content.parts[0].text;
        console.log(`[AI] ✅ Gemini успешно! Длина ответа: ${response.length} символов`);
        return response;
      }
      
      const err = await r.text();
      console.error("[AI Gemini Error]", r.status, err.slice(0, 200));
    } catch(e) { 
      console.warn("[AI Gemini Exception]", e.message); 
    }
  }
  
  console.error("[AI] ❌ Оба API недоступны!");
  return null;
}
